...
from app.persistence.models import RawMetric, ResampleMetricMinute, ResampleState
...
class RetentionService:
    ...
    def __init__(self, session: AsyncSession, retention_raw_days: int = 90, retention_minute_days: int = 90):
        self.session = session
        self.retention_raw_sec = retention_raw_days * 86400
        self.retention_minute_sec = retention_minute_days * 86400
    ...
    async def cleanup_minute(self) -> int:
        """Delete minute aggregates older than retention period."""
        cutoff = int(time.time()) - self.retention_minute_sec
        stmt = delete(ResampleMetricMinute).where(
            ResampleMetricMinute.timestamp < cutoff
        )
        result = await self.session.execute(stmt)
        await self.session.commit()
